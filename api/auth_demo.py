...
class H(BaseHTTPRequestHandler):
    ...
    def who_session(self):
        ...
        return rec
    SCHEMES = {"basic": who_basic, "apikey": who_apikey, "token": who_token, "jwt": who_jwt, "hmac": who_hmac, "session": who_session}
    ...
    def guarded(self):
        ...
        if self.command == "DELETE":                                    # 403: I know you, and no
            if role != "teacher": return self.send(403, {"error": {"code": "forbidden", "message": f"{user} is a {role}: may read grades, not delete them"}})
    ...
    def do_POST(self):
        if urlparse(self.path).path != "/login": return self.send(404, {"error": "POST /login"})
...
if __name__ == "__main__":
    ...
    print(f"🪪 six hall passes on http://127.0.0.1:{port} — POST /login · GET /<basic|apikey|token|jwt|hmac|session>/grades", file=sys.stderr, flush=True)
